...
export default function MyItemsPage() {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get(`${config.baseURL}/item/user/${user._id}`, { withCredentials: true });
  ...
  const handleResolve = async (id) => {
    if (!window.confirm("Mark as resolved and remove this report?")) return;
    try {
      await axios.delete(`${config.baseURL}/item/${id}`, { withCredentials: true });
      toast.success("Report removed");
      fetchItems();
    } catch { toast.error("Failed to remove report"); }
  };

  useEffect(() => { if (user?._id) fetchItems(); }, [user]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">All your submitted reports</p>
          </div>
          <Link to="/report" className="btn-primary self-start sm:self-auto">➕ New Report</Link>
        </div>
        ...
        {loading ? (
          ...
          <EmptyState icon="📋" title="No reports yet"
            ...
            action={<Link to="/report" className="btn-primary">Submit first report</Link>} />
        ) : (
